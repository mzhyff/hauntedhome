...
public class worldbuild : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        */
        ...
        for (float xPos = -72; xPos < 72; xPos += 12)
        {

            for (float zPos = 4; zPos < 88; zPos += 12)
            {
                ...
                if (spawnchance > 4 && spawnchance < 6)
                {
                    Instantiate(img1, new Vector3(xPos, Random.Range(4, 12), zPos), Quaternion.Euler(0, Random.Range(0, 2) * 90, 0));
                }
                // end is the randomised spawned img1

                // below is the randomised spawned beda
             
                if (spawnchance > 7 && spawnchance < 9)
                {
                    Instantiate(beda, new Vector3(xPos, 4, zPos), Quaternion.Euler(0, Random.Range(0, 2) * 90, -5));
                }
                // end is the randomised spawned beda

                // below is the randomised spawned tv

                if (spawnchance > 10 && spawnchance < 12)
                {
                    Instantiate(tv, new Vector3(xPos, 4, zPos), Quaternion.Euler(0, Random.Range(0, 2) * 90, -8));
                }
        ...
        for (float xPos = -68; xPos < 76; xPos += 12)
        {
            ...
            for (float zPos = 4; zPos < 80; zPos += 12)
            {
                ...
                if (spawnchance > 7 && spawnchance < 20)
                {
                    Instantiate(wall, new Vector3(xPos, 4, zPos), Quaternion.Euler(0, Random.Range(0, 2) * 90, 0));
                }
                // ende randomised spawned walls

                spawnchance = Random.Range(1, 16);
                if (spawnchance > 1 && spawnchance < 6)
                {
                    Instantiate(door, new Vector3(xPos, 4, zPos), Quaternion.Euler(0, Random.Range(0, 2) * 90, 0));
                }
                // ende randomised spawned door
            }
        }
        ...
        for (float xPos = -68; xPos < 76; xPos += 12)
        {
            Instantiate(wall, new Vector3(xPos, 4, 4), wall.rotation);
            Instantiate(wall, new Vector3(xPos, 4, 88), wall.rotation);
        }

        for (float zPos = 4; zPos < 80; zPos += 12)
        {
            Instantiate(wall, new Vector3(-68, 4, zPos), Quaternion.Euler(0, 90, 0));
            Instantiate(wall, new Vector3(76, 4, zPos), Quaternion.Euler(0, 90, 0));
        }
